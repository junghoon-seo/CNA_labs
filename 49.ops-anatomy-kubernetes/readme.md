...
2. Connect to VM (here, aws EC2 Linux-AMI)
```
ssh -i ./k8s.pem ubuntu@ec2-3-35-209-101.ap-northeast-2.compute.amazonaws.com
```

3. Set root passwd and switching User to root
```
sudo passwd root
su -
```
...
4. kubectl 이 실행되는지 확인한다:
```
kubectl
```

5. 압축파일이 더 이상 필요없으니 삭제:
```
